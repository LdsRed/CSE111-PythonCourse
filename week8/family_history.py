...
def print_death_age(people_dict):
    ...
    for item in people_dict.items():
        people = item[1]
        age = people[DEATH_YEAR_INDEX] - people[BIRTH_YEAR_INDEX]
        name = people[NAME_INDEX]
        print(f"{name} {age} \nYear of birth: {people[BIRTH_YEAR_INDEX]} \nYear of death: {people[DEATH_YEAR_INDEX]}")
        print()
...
def count_genders(people_dict):
    ...
    print("Genders")
    for item in people_dict.items():
        people_information = item[1]
        if people_information[GENDER_INDEX] == "F":
            count_female += 1
        elif people_information[GENDER_INDEX] == "M":
            count_male += 1
